...
async execute(message, args, client) {
    ...
    if (!channel) return attentionembed(message, "Je moet in een spraakkanaal zitten om muziek te kunnen luisteren");
    //If not in the same channel return error
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return attentionembed(message, `Je moet in hetzelfde spraakkanaal zitten als mij`);
    //If no args return
    if (!args.length)
    ...
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Tis ongeheurd dat ik je kanaal niet in kan komen!");
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Zoeken 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Zoeken 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
       //If nothing is playing join the channel
        ...
        //send join message
        message.channel.send(new MessageEmbed().setColor("#F0EAD6")
          .setDescription(`**👍 Ah ha betrapt in \`${channel.name}\` 📄 bound \`#${message.channel.name}\`**`)
