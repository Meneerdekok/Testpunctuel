...
const Radiostations = [
  ...
  "Sky-radio http://23613.live.streamtheworld.com/SKYRADIO.mp3",//netherlands
...
**34: ** [\`${Radiostations[34 - 1].split(" ")[0]}\`](${Radiostations[34 - 1].split(" ")[1]})`, inline: true
    ...
    if (!channel) return attentionembed(message, "Je zit niet in een spraakkanaal");
    //react with emoji
    message.react(approveemoji);
    //If not in the same channel return error
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return attentionembed(message, `Je moet in hetzelfde kanaal zitten als mij`);
    //check permissions
    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Ik heb geen premissies om je kanaal in te komen");
    ...
    if (isNaN(args[0])) {
      ...
      return message.reply(
        new MessageEmbed()
          .setColor("#F0EAD6")
          .setTitle(`Dit is geen geldige radio geef een nummer tussen\`1\` en \`${Radiostations.length}\``)
      );
    }
    ...
    message.channel.send(new MessageEmbed().setColor("#F0EAD6")
      .setDescription(`**:radio: Aan het zoeken 🔍 \`${Radiostations[i - 1].split(" ")[0]}\`**`));
    //mute yourself
    ...
    */
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .addField("Positie in de queue", `**\`${serverQueue.songs.length - 1}\`**`, true)
        .setFooter(`Aangevraagt door: ${message.author.username}#${message.author.discriminator}`, message.member.user.displayAvatarURL({ dynamic: true }))
    ...
    try {
      ...
      await channel.leave();
      return message.channel.send(`Kon het kanaal in komen: ${error}`).catch(console.error);
    }
    //sende bestätigung
  }
};
